...
{
    //if (row < 0 || col < 0 || row + k > m || col + k > n) return false;
...
{
    if (3 * k > n || 3 * k > m) return false; // на всякий оставим...
    ...
    for (int i = k; i <= count_place_row.size() - 2 * k; i++) // есть смысл начинать с k строки, так как сверху есть квадратик и заканчивать так же
    {
        if (count_place_row[i] >= 3 * k) // эта проверка не обязательна, но пусть будет (меньше вызовов функции будет)
        {
            if (test_inner_new(count_place_row, k, i)) // проверка k соседей снизу на наличие подходящего количества закрашенных клеток
            {
                ...
                // проскользим окном по этой строке
                //if (window_new(m, n, map, k, i)) return true; // если проскользили по строке и нашли крест, то возвращаем истину
            }
        }
    }
...
bool test_variant6(int k, const int m, const int n, const vector<vector<char>>& map, const vector<vector<Point>>& points)
{
    ...
    for (int i = k; i <= m - 2 * k; i++) // есть смысл начинать с k строки, так как сверху есть квадратик и заканчивать так же
    {
        for (int j = k; j <= n - 2 * k; j++) // есть смысл начинать с k строки, так как сверху есть квадратик и заканчивать так же
        {
            ...
            {
                // проверка на такое же условие для остальных точек в линии
                //cout << i << " " << j << endl;
...
int main()
{
    ...
    //rows.resize(m);
    //// для каждой точки подсчитаем вес по горизонтали
    //for (int i = 0; i < m; i++)
    //{
    //    rows[i].resize(n, 0);
    //
    ...
    //}
    //
    //cout << bin_search_variant5(m, n, map, rows, cols) << endl;
    ...
    // старый варик
    // 
    //vector<int> count_place_row;
    //count_place_row.resize(m, 0);
    //// подсчет того, сколько в строках закрашенных мест - фейл
    //// подсчет максимально длинного закрашенного места в строке
    //for (int i = 0; i < m; i++)
    //{
    //    //bool flag = false;
    //    int max = 0;
    ...
    //
    //    if (max < sum) max = sum;
    //    count_place_row[i] = max;
    //}
    //
    //vector<int> count_place_col;
    //count_place_col.resize(n, 0);
    //
    //// подсчет того, сколько в строках закрашенных мест - фейл
    //// подсчет максимально длинного закрашенного места в строке
    //for (int i = 0; i < n; i++)
    ...
    //
    //    if (max < sum) max = sum;
    //    count_place_col[i] = max;
    //}
    //
    //cout << endl;
    //cout << bin_search(m, n, map, count_place_row, count_place_col) << endl;
